...
\texttt{adminOnly} middleware on administrative routes, and the Multer upload
...
\midrule
\texttt{/api/auth} & Register, verify and resend e-mail verification, login,
forgot/reset password, current user, profile update, password change. \\
\texttt{/api/users} & Public user lookup, favorites toggle, avatar upload. \\
\texttt{/api/pets} & Public pet listing, featured pets, own pets, pet CRUD with
owner enforcement, QR generation. \\
\texttt{/api/breeds} & Public breed catalogue; admin create/update/delete. \\
\texttt{/api/adoptions} & Adoption applications, own applications, admin listing
and status updates with pet transition and notification. \\
\texttt{/api/lost-found} & Public reports, owner CRUD, admin status updates and
deletion. \\
\texttt{/api/health} & Owner-scoped health records per pet. \\
\texttt{/api/vaccinations} & Owner-scoped vaccination records and upcoming list. \\
\texttt{/api/favorites} & Owner-scoped favourite pets (unique user--pet pairs). \\
\texttt{/api/veterinarians} & Public directory; admin CRUD. \\
\texttt{/api/appointments} & Booking with slot conflict check, auto-notification
and auto-reminder, reschedule and cancel with reminder sync. \\
\texttt{/api/community} & Public posts, owner CRUD, likes, comments; admin
moderation. \\
\texttt{/api/ai} & \texttt{/ask} (PetGPT over Gemini with fallback) and
\texttt{/advice} (rule-based per-pet advice). \\
\texttt{/api/notifications} & Owner-scoped notification read, unread, mark
read/all-read, delete. \\
\texttt{/api/reminders} & Owner-scoped reminders CRUD and completion. \\
\texttt{/api/admin} & Dashboard statistics, user/pet/lost-found/community
administration. \\
\bottomrule
\end{longtable}
\end{center}
...
uploaded images resolve from the API.
...
\texttt{MONGODB\_URI}, defaulting to \texttt{mongodb://localhost:27017/
animal\_planet}.