...
The system is built as a client--server application. The backend is a Node.js
application written with Express that exposes a REST API and persists all data in
MongoDB through the Mongoose object-document mapper. The frontend is a web
application written in JavaScript. At the time this black book was prepared, the
frontend is being migrated to a React and TypeScript implementation inside
\texttt{frontend-react} as an incremental, phase-by-phase effort; the backend
remains the source of truth for all data and business rules.
...
the remaining React migration phases, is explicitly identified as planned rather
than as delivered functionality.
...
The context diagram shows four external parties. The \emph{Web client} interacts
with the platform through the public HTTP API; the \emph{MongoDB} database stores
...
scope. References, a glossary, and appendices conclude the black book.