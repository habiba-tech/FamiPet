...
sent through Nodemailer~\cite{nodemailer}; if the e-mail cannot be sent the user document is
...
password, compares the presented password with the stored
bcrypt~\cite{bcryptjs} hash, refuses
blocked and unverified accounts, and on success signs a
JWT~\cite{rfc7519} containing the user
...
(\path{backend/middleware/auth.js}). Figure~\ref{fig:access} shows its decision
tree.
...
\path{req.user.role === "admin"} for administrative routes, returning 403
...
Security headers & Helmet~\cite{helmet} headers with \texttt{crossOriginResourcePolicy:
