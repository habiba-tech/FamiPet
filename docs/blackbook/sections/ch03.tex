...
(routes, controllers, models, middleware, config) and a phase-based migration
...
\begin{itemize}
    \item the OpenAI adapter advertised in \texttt{backend/.env.example}
    (\texttt{PETGPT\_PROVIDER=openai}); the implemented assistant uses only the
    ...
    \item the React migration phases 22--29 (AI redesign, API integration layer,
    authentication consolidation, UI completion, visual/functional regression,
    Docker/Nginx integration, and removal of the vanilla frontend).
\end{itemize}
